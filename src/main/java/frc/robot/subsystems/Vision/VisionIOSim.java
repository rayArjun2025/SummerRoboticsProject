...
// Raymond: this is the reference's VisionIOSimPhoton renamed to VisionIOSim and reworked for two
// cameras. the rename is fine as long as it's consistent (Vision.java references VisionIOSim), just
// flagging so you know where it came from.
...
public class VisionIOSim implements VisionIO {
  ...
  public VisionIOSim(Transform3d leftTransform3d, Transform3d rightTransform3d) {
    ...
    // Raymond: you hardcoded k2025ReefscapeAndyMark. reference loads kDefaultField (and ships a
    // 2026-rebuilt-andymark.json you didn't copy over). fine for now since we're on reefscape, but
    // know it's pinned to one field.
    ...
    // Raymond: 880 - reference is 1280x800. did you mean 800? if you copied a real camera's res
    // that's fine, but if it's a typo your sim calibration is off.
    ...
    lPoseEstimator =
        new PhotonPoseEstimator(
            ...
            this.leftTransform3d);
    rPoseEstimator =
        new PhotonPoseEstimator(
            reefField,
            PhotonPoseEstimator.PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR,
            this.rightTransform3d);
  ...
  public void updateInputs(VisionIOInputs inputs) {
    ...
    if (rightHasTargets) {
      ...
      if (bestTarget != null && rightBest != null) {
        ...
        double leftDistance = bestTarget.getBestCameraToTarget().getTranslation().getNorm();

        double rightDistance = rightBest.getBestCameraToTarget().getTranslation().getNorm();
  ...
  private void addObservation(List<PoseObservation> observations, EstimatedRobotPose est) {
    ...
    observations.add(
        new PoseObservation(
            ...
            avgDist,
            megaTag1 ? PoseObservationType.MEGATAG_1 : PoseObservationType.MEGATAG_2));
  }
