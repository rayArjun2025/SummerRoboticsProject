...
// Raymond: this class is generated by @AutoLog at build time - you don't hand-write or check it in. delete the file and let the annotation make it (that's the whole point of the @AutoLog on ElevatorIOInputs). a hand-rolled copy will drift from the real fields, which is already happening - see `connected` below.
public class ElevatorIOInputsAutoLogged extends ElevatorIO.ElevatorIOInputs
  ...
  public void toLog(LogTable table) {
    ...
    // Raymond: `connected` exists on the inputs and Real sets it, but you never log/restore/clone it here - so it's invisible in logs and lost on replay. another reason to just let @AutoLog generate this.
  }
