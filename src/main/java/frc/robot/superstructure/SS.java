...
// Raymond: ShoulderState is singular but everything else is plural - ElbowStates, HandStates,
// ClimberStates, ElevatorStates. rename it ShoulderStates so it matches.
...
// Raymond: run spotlessApply - missing space before the brace, and there's stray double blank lines
// and trailing whitespace all through this file.
public class SS extends StateMachineSubsystemBase<SuperstructureStates> {
  private static SS instance;

  // Raymond: these setpoints belong in each subsystem's own Constants file (see how the reference
  // pulls ShooterConstants/ClimberConstants), not dumped in SS. and _POS isn't a unit - name it _m
  // or whatever the elevator actually reads in.
  ...
  // Raymond: this whole Flag-bag isn't how we do it. look at the reference - it splits Intention
  // (what the driver asks for) from InternalState (what the machine's actually doing), and
  // intend(Intention) + handleIntention() maps one to the other. here you've got a pile of booleans
  // you cascade through, so two flags can be set at once and whoever's first in the if-chain wins.
  // switch to the intention pattern.
  ...
  public void handleStateMachine() {
    ...
    // Raymond: nothing in this cascade ever queues INTAKE_CORAL, so that case in the switch below
    // is dead - you can't reach it. wire a flag/intention to it or drop it.
    if (has(Flag.CLIMB)) {
      queueState(SuperstructureStates.CLIMBING);
    } else if (has(Flag.HOME)) {
      queueState(SuperstructureStates.STOWED);
      // Raymond: this branch just queues STOWED and does nothing - manual up/down are wired to
      // nothing. either implement them or delete the flags. and kill the random blank lines inside.
    } else if (has(Flag.MANUAL_UP) || has(Flag.MANUAL_DOWN)) {

      queueState(SuperstructureStates.STOWED);

      // Raymond: SCORE_HIGH and SCORE_LOW both go to SCORE, and scoreElevatorTarget is always
      // SCORE_ELEVATOR_POS - so high vs low is identical, the distinction does nothing. the
      // reference parameterizes the setpoint off the intention. do that instead of two flags that
      // behave the same.
    ...
    switch (getState()) {
      ...
      case INTAKE_CORAL:
        ...
        // Raymond: you drive hand with requestState() but elevator/shoulder/elbow with
        // setTargetPosition/setTargetAngle - pick one way to talk to subsystems. the reference just
        // calls queueState() on the subsystem directly, the requestState wrapper is pointless
        // indirection.
  ...
  private void handleClimbing() {
    ...
    // Raymond: using isState(IDLE) to mean "got there" is fragile - a subsystem sits in IDLE for
    // other reasons too. the reference checks isValueReached(tolerance). use that here and in
    // atScorePosition().
    boolean clearToClimb =
        elevator.isState(ElevatorStates.IDLE)
  ...
  private void handleScoring() {
    ...
    switch (scoringSubstate) {
      ...
      case SETTLING:
        ...
        // Raymond: READY just holds position - nothing ever tells the hand to release/score the
        // piece. so SCORE never actually scores anything. finish it.
      case READY:
        driveToScorePosition();
        if (!atScorePosition()) {
          setScoringSubstate(Scoring.RAISING);
